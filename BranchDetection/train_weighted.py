...
val_stacks = [0]
# train_stacks = [0,1,2]
train_stacks = list(set(list(range(16))) - set(val_stacks) - set([6, 14]))
save_folder = './U_intensity_att1_k4/'
...
for epoch in range(parameters.num_epoch):
    ...
    for stack in (train_stacks):
        ...
        for i, (inputs, targets, weights) in enumerate(train_data):           
            ...
            optimizer.zero_grad()
            yhat = model.train_forward(inputs)

            
            
            loss = sumed_iou_coef(yhat, targets+ weights.to(device = parameters.device)*200, smooth = 1e-2)
            
             

            print(stack, i, "loss",loss.item())
            ...
            del inputs, targets, weights, yhat
    ...
    for stack in (val_stacks):
        val_data = DataGenerator(parameters.Im_path,parameters.label_path,parameters.points_path,parameters.weights_path,  
                            stack_ind_aug = [stack,0,0,0],
                            batch_size = parameters.num_batch, 
                            dim_in = (32,32,8,1), 
                            dim_out = (32,32,8,2), 
        ...
        for i, (inputs, targets, weights) in enumerate(val_data):           
            ...
            val_loss = sumed_iou_coef(yhat, targets+  weights.to(device = parameters.device)*200, smooth = 1e-2)
    ...
    print(epoch,training_history[epoch]['loss'],training_history[epoch]['val_loss'])
